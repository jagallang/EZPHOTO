...
### ✨ 새로운 기능
- **🖼️ 로고 업로드**: 견적서 좌측 상단 로고 클릭으로 카메라/갤러리에서 이미지 업로드
- **📝 맞춤형 편집**: 각 텍스트 필드별 전용 편집 다이얼로그 (Company, Tel, Address 등)
- **📋 테이블 편집**: 견적서 테이블 각 셀 개별 편집 (Description, Specification, Unit, Qty, Price, Remarks)
- **🔢 견적서 번호**: Est. No. 클릭으로 견적서 번호 입력 및 수정 가능
- **🎯 실시간 반영**: 모든 편집 내용이 즉시 UI에 반영되어 직관적 사용성 제공

### 🐛 버그 수정
- **이미지 최적화**: 로고 이미지 자동 리사이즈 (300x300) 및 품질 최적화 (85%)
- **편집 안정성**: 필드별 데이터 바인딩 개선으로 편집 중 데이터 손실 방지
- **다이얼로그 개선**: PhotoSourceDialog 통합으로 일관된 이미지 선택 경험
- **메모리 효율화**: base64 인코딩 최적화로 로고 이미지 메모리 사용량 개선

### 🎯 사용성 향상
- **직관적 필드명**: 편집 다이얼로그에서 "Company 편집", "Tel 편집" 등 명확한 라벨 표시
- **원클릭 편집**: 로고, 텍스트, 테이블 셀 모든 요소 클릭 한 번으로 편집 시작
- **전문적 완성도**: 비즈니스 견적서에 필요한 모든 요소 완벽 편집 지원
- **실시간 미리보기**: 편집 즉시 결과 확인으로 효율적 작업 환경 구현
...
### 🎯 v1.2.12 (다음 릴리스)
- [ ] 견적서 데이터 영구 저장 기능 (로컬 스토리지)
- [ ] 견적서 템플릿 추가 스타일 옵션 (색상, 폰트)
- [ ] PDF 견적서 자동 생성 및 다운로드 기능
- [ ] 견적서 템플릿 사전 설정 저장/불러오기
