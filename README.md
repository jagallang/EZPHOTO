...
### ✨ 새로운 기능
- **📄 A4 비율 이미지 저장**: 견적서 이미지를 표준 A4 용지 비율(1:1.414)로 최적화하여 저장
- **🖨️ 인쇄 품질 향상**: A4 규격 (595x842 points) 적용으로 프로페셔널한 문서 품질 제공
- **📐 고해상도 캡처**: pixelRatio 3.0-6.0 적용으로 초고화질 이미지 생성
- **🎯 화면 일치성**: 웹 화면 표시와 저장 결과물의 완벽한 일관성 달성
- **⚡ 최적화된 레이아웃**: MainAxisSize.min 적용으로 불필요한 여백 완전 제거

### 🐛 버그 수정  
- **📏 비율 오류 수정**: 기존 정사각형 저장 문제를 A4 표준 비율로 완전 해결
- **🔧 레이아웃 개선**: CoverPageWidget의 Column에 MainAxisSize.min 적용으로 공간 최적화
- **📱 반응형 개선**: 웹과 모바일에서 동일한 A4 비율 보장
- **💾 고품질 보존**: 해상도 향상과 동시에 올바른 종횡비 유지

### 🎯 사용성 향상
- **🏢 비즈니스 표준**: A4 용지 규격으로 실제 인쇄 시 완벽한 문서 품질
- **📋 전문성 강화**: 견적서 저장 결과가 실제 비즈니스 문서 표준에 부합
- **🎨 시각적 완성도**: 화면에서 보는 것과 저장되는 결과물의 완벽한 일치
- **⚙️ 기술적 우수성**: Flutter 위젯 렌더링과 이미지 캡처의 최적화 구현
